...
(defun emacs-superset-diff-review (workspace)
  ...
  (interactive (list (emacs-superset--read-workspace "Review diff for: ")))
...
(defun emacs-superset-diff-uncommitted (workspace)
  ...
  (interactive (list (emacs-superset--read-workspace "Uncommitted changes for: ")))
  (let ((default-directory (emacs-superset-workspace-path workspace)))
    (magit-diff-unstaged)))

(defun emacs-superset-diff-staged (workspace)
  "Show staged changes in WORKSPACE."
  (interactive (list (emacs-superset--read-workspace "Staged changes for: ")))
  (let ((default-directory (emacs-superset-workspace-path workspace)))
    (magit-diff-staged)))
